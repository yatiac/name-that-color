...
module.exports = function name(color) {
    ...
    if (color.length < 3 || color.length > 7)
        return {exactMatch:false, closestColor: `#000000`, colorName: "Invalid Color: " + color};
    if (color.length % 3 == 0)
        color = "#" + color;
    if (color.length == 4)
    ...
    for (var i = 0; i < names.length; i++) {
        if (color == "#" + names[i][0]) {
            return {exactMatch: (color == "#" + names[i][0]),colorName:  names[i][1],closestColor: `#${names[i][0]}`}            
        }
    ...
    return {exactMatch: false,colorName: names[cl][1],closestColor: `#${names[cl][0]}`}

    return (cl < 0 ? { code: "#000000", name: "Invalid Color: " + color, } : {code: "#" + names[cl][0], name: names[cl][1]});
}

const init = function () {    
    for (var i = 0; i < names.length; i++) {
        let colorVal = "#" + names[i][0];
        let rgbVal = rgb(colorVal);
        let hslVal = hsl(colorVal);
        names[i].push(rgbVal[0], rgbVal[1], rgbVal[2], hslVal[0], hslVal[1], hslVal[2]);
    }
}
